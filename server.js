...
app.delete("/api/projects/:id",auth,(req,res)=>{try{const ps=read();const p=ps.find(x=>x.id===req.params.id);if(!p)return res.status(404).json({error:"Not found"});for(const u of [p.videoUrl,p.modelUrl,p.thumbnail])if(u){const f=path.join(__dirname,u.replace(/^\//,""));if(fs.existsSync(f))fs.unlinkSync(f)}write(ps.filter(x=>x.id!==p.id));res.json({ok:true})}catch{res.status(500).json({error:"Delete failed"})}});

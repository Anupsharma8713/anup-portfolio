...
app.post("/api/login",(req,res)=>{if(req.body.password!==ADMIN_PASSWORD)return res.status(401).json({error:"Wrong password"});const token=crypto.randomBytes(24).toString("hex");tokens.add(token);res.json({token})});
function auth(req,res,next){const t=(req.headers.authorization||"").replace("Bearer ","");if(!tokens.has(t))return res.status(401).json({error:"Please login again"});next()}
...
app.delete("/api/projects/:id",auth,(req,res)=>{const ps=read();const p=ps.find(x=>x.id===req.params.id);if(!p)return res.status(404).json({error:"Not found"});for(const u of [p.videoUrl,p.modelUrl,p.thumbnail])if(u){const f=path.join(__dirname,u.replace(/^\//,""));if(fs.existsSync(f))fs.unlinkSync(f)}write(ps.filter(x=>x.id!==p.id));res.json({ok:true})});
